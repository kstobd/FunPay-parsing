...
def send_welcome(message):
    bot.reply_to(message, f'Я бот по отслеживанию цен на сайте FunPay. Приятно познакомитьс{message.from_user.first_name}. Напшиши /help')
@bot.message_handler(commands=['help'])
def send_welcome(message):
    bot.reply_to(message, f'Достпуные команды: \n url, чтобы ввести новый адрес сканирования \namount,чтобысоздатьограничение на колличество товара \n price, чтобы создать ограничение на цену')
...
def get_text_messages(message):
    if message.text.lower() == 'привет':
        ...
        bot.send_message(message.from_user.id, 'Отправьте максимальное значение цены на товар (если числовещественное,тзаписывайте его через точку (0.8))')
...
def get_price(message):
    try:
        ...
        bot.send_message(message.from_user.id, 'Максимальная цена успешно обновлена')
    except BaseException:
        bot.send_message(message.from_user.id, "Ты что-то ввел не правильно. Введи еще раз")
        bot.register_next_step_handler(message, get_price)
